 ...
 */
public class ItemNameColorCommand implements CommandExecutor {
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Plugin.getInstance(), new Runnable() {
            @Override
            public void run() {
                if (Utils.canChangeName(player, hand, true)) {
                    Utils.nameColorItem(player, hand, Utils.getNameFromArgs(args));
                }
            }
        });
        return true;
